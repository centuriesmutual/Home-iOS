// MARK: - New File: RadioStation.swift
import Foundation

/// Model representing a radio station with location and streaming information
...
// MARK: - New File: LocationManager.swift
import Foundation
import CoreLocation
import Combine

/// Manages location services for radio station detection
@MainActor
...
// MARK: - New File: StationFinder.swift
import Foundation
import CoreLocation

/// Finds the nearest radio station based on user location
...
// MARK: - New File: RadioPlayer.swift
import Foundation
import AVFoundation
import MediaPlayer
import Combine

/// Manages radio streaming and playback controls
@MainActor
...
// MARK: - Updated File: ios_app_screens.swift (HomeScreen section only)
// Note: This shows only the HomeScreen modifications. Keep your existing ContentView and other screens unchanged.
...
struct HomeScreen: View {
    ...
    var body: some View {
        ZStack {
            ...
            // Bottom action buttons (modified to include radio)
            VStack {
                Spacer()
                
                HStack(spacing: 30) {
                    // Mail button (existing)
                    ActionButton(
                        icon: "envelope",
                        label: "Mail",
                        action: { currentScreen = "mailbox" }
                    )
                    
                    // Cloud Network button (existing)
                    ActionButton(
                        icon: "cloud",
                        label: "Cloud Network",
                        action: { currentScreen = "cloudNetwork" }
                    )
                    
                    // Radio button (new)
                    ActionButton(
                        icon: isRadioEnabled ? "radio.fill" : "radio",
                        label: "Radio",
                        isActive: isRadioEnabled,
                        action: { toggleRadio() }
                    )
    ...
    private func handleLocationUpdate(_ location: CLLocation?) {
        ...
        if stationFinder.updateNearestStation(for: location) {
            // Station changed, update player if currently playing
...
// MARK: - Action Button Component (Enhanced)
...
<string>Centuries Mutual uses your location to automatically find and play nearby public radio stations, enhancing your listening experience based on your current region.</string>
...
<dict>
    ...
    <dict>
        <key>streams.kqed.org</key>
        <dict>
            <key>NSExceptionAllowsInsecureHTTPLoads</key>
            <true/>
        </dict>
        <key>fm939.wnyc.org</key>
        <dict>
            <key>NSExceptionAllowsInsecureHTTPLoads</key>
            <true/>
        </dict>
...
/*
1. Add all new Swift files (RadioStation.swift, LocationManager.swift, StationFinder.swift, RadioPlayer.swift) to your Xcode project.

2. Update your existing HomeScreen in ios_app_screens.swift with the modified version above.

3. Add the Info.plist keys as shown in the comments.

4. The radio feature will:
   - Request location permission when first enabled
   - Automatically find the nearest station based on location
   - Switch stations when moving >50km from current station
   - Support background playback with lock screen controls
   - Handle errors gracefully with user-friendly messages

5. The UI integrates seamlessly with your existing dark theme and layout.

6. All code follows Swift best practices with proper error handling, @MainActor usage, and modular design.
*/
...
class SQLManager: ObservableObject {
    ...
    private func syncEnrollmentUpdateToDropbox(enrollmentId: String, status: String) {
        let updateData = [
        ...
        let updateJSON = try! JSONEncoder().encode(updateData)
        let folderPath = "\(DropboxManager.Config.Folders.enrollments)/\(enrollmentId)"
        ...
        DropboxManager.shared.uploadFile(
            ...
            metadata: [
                DropboxManager.Config.Metadata.enrollmentId: enrollmentId,
                DropboxManager.Config.Metadata.status: status
            ],
            enrollmentId: enrollmentId
        )
        .sink(
            receiveCompletion: { completion in
                if case .failure(let error) = completion {
                    print("Failed to sync enrollment update: \(error)")
                }
            },
            receiveValue: { _ in
                print("Enrollment status update synced to Dropbox")
            }
        )
        .store(in: &cancellables)
    }
...
enum SQLError: LocalizedError {
    ...
    func updateEnrollmentStatus(enrollmentId: String, status: String) -> AnyPublisher<Bool, Error> {
        return Future { [weak self] promise in
            self?.dbQueue.async {
                guard let self = self else {
                    promise(.failure(SQLError.databaseNotAvailable))
                ...
                if sqlite3_prepare_v2(self.db, updateSQL, -1, &statement, nil) == SQLITE_OK {
                    ...
                    sqlite3_bind_text(statement, 2, currentTime, -1, nil)
                    sqlite3_bind_text(statement, 3, enrollmentId, -1, nil)
                    
                    if sqlite3_step(statement) == SQLITE_DONE {
                        // Sync update to Dropbox
                        self.syncEnrollmentUpdateToDropbox(enrollmentId: enrollmentId, status: status)
                        promise(.success(true))
                    } else {
                        let errmsg = String(cString: sqlite3_errmsg(self.db)!)
                        promise(.failure(SQLError.updateFailed(errmsg)))
                    }
                } else {
                    let errmsg = String(cString: sqlite3_errmsg(self.db)!)
                    promise(.failure(SQLError.queryPreparationFailed(
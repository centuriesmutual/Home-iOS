...
struct Transaction: Identifiable, Codable {
    ...
    init(id: UUID = UUID(), description: String, amount: Double, date: String, category: TransactionCategory, status: TransactionStatus) {
        self.id = id
...
struct WalletView: View {
    ...
    private func fetchBalances(completion: @escaping (Result<WalletBalances, Error>) -> Void) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
            let mockBalances = WalletBalances(
                evergreen: 1250.75,
                steps: 340.25,
                adRevenue: 89.50
            )
            completion(.success(mockBalances))
        }
    }
    ...
    private func fetchTransactions(completion: @escaping (Result<[Transaction], Error>) -> Void) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.2) {
            let mockTransactions = [
                ...
                Transaction(
                    description: "Daily Steps Reward",
                    amount: 2.50,
                    date: "2024-01-14",
                    category: .steps,
                    status: .completed
    ...
    private func fetchHealthScore(completion: @escaping (Result<FinancialHealthScore, Error>) -> Void) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
            let mockScore = FinancialHealthScore(
                score: 75,
                level: "Good Progress",
                nextGoal: "Reach $2000 total balance"
            )
